...
module.exports = function(req, res){
  con.connect(function(err){
     con.query("SELECT lozinka FROM korisnik WHERE email=?;",[req.session.podaci.email], function (err, result, fields) {
         ...
         if(result){
           if(req.body.novaLozinka === req.body.novaLozinkaPon){
             bcrypt.hash(req.body.novaLozinka, 10, function(err, hash){
               con.query("UPDATE korisnik SET lozinka = ? WHERE email = ?",[hash, req.session.podaci.email], function (err, result, fields) {
